...
async function loadRuntimeConfig(rootDir, env) {
  const localConfig = await readJsonIfExists(path.join(rootDir, 'js', 'private-config.local.json'));
  const publicConfig = await readJsonIfExists(path.join(rootDir, 'js', 'config.json'));
...
async function loadStaticManifest(rootDir) {
  const manifest = await readJsonIfExists(path.join(rootDir, 'manifest.json'));
  return (manifest && manifest.articles) || [];
}

async function loadStaticArticleBySlug(rootDir, slug) {
  const articles = await loadStaticManifest(rootDir);
  ...
  return Promise.all(
    articles.map(async function (article) {
      ...
      if (includeHtml && article.folder) {
        next.html = await fs.readFile(path.join(rootDir, article.folder, 'index.html'), 'utf8');
      }
...
async function resolveArticleBySlug(rootDir, config, slug) {
  ...
  return loadStaticArticleBySlug(rootDir, slug);
}
...
async function readBundledSummary(rootDir, slug) {
  const payload = await readJsonIfExists(path.join(rootDir, SUMMARY_DIR, slug + '.json'));
  return normalizeSummaryRecord(payload);
}
