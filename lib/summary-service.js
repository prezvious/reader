...
async function loadRuntimeConfig(rootDir, env, options) {
  ...
  return {
    openRouterApiKey: String((env && env.OPENROUTER_API_KEY) || (localConfig && localConfig.openRouterApiKey) || '').trim(),
    ...
    supabase: {
      ...
      serviceRoleKey: String((env && (env.SUPABASE_SERVICE_ROLE_KEY || env.READER_SUPABASE_SERVICE_ROLE_KEY)) || (localConfig && localConfig.serviceRoleKey) || '').trim()
    }
  };
}
